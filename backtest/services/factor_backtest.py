...
# from . import Metric
from core.scaling import annualize_scaler

from price.services import convert_freq, add_cash, rebal_dates, price_on_rebal, calculate_portvals, port_rets

from strategy.services.factors import BetaFactor, MomentumFactor, VolatilityFactor
# ProphetFactor

from strategy.services.optimize import Equalizer, Optimization, TimeSeries
...
class FactorBacktest:
    ...
    def factor_signal(self, factor) -> pd.DataFrame:
        ...
        if self.factor == 'beta':
            #no problem
            factor_signal = class_instance(
                equity_with_benchmark=self.daily_price_df,
            ...
            with open(str(settings.BASE_DIR) + static('pickle/prophet_signal.pickle'), 'rb') as f:
    ...
    def cross_weight(self) -> pd.DataFrame:
        ...
        return self.cs_instance(
            self.signal, self.daily_price_df, 
            self.rebal_freq, self.cs_model).weight()
        
    def time_weight(self) -> pd.Series:
        return self.ts_instance(
            self.cs_port_cum_rets, 
            self.cs_weight, 
            self.risk_tolerance, 
            self.ts_model).weight()

    def port_return(
        self, 
        weight_name: str='cs_weight',
        freq: str='D', 
        cumulative: bool=True, 
        long_only: bool=True, 
        yearly_rfr: float=0.03
    ) -> pd.Series:
        """_summary_
        ...
        if weight_name == 'cs_weight':
            ...
            port_value = calculate_portvals(
                self.daily_price_df, weight, 
            ...
            port_value = calculate_portvals(
                price_df, ts_cs_weight, self.signal, long_only)
...
if __name__ == '__main__':
    ...
    all_assets_df = pd.read_csv(
        str(settings.BASE_DIR) + static('csv/alter_with_equity.csv'), index_col=0)
