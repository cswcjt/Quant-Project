from . import FactorBacktest, Metric
from price.services import (
    add_cash, rebal_dates, 
    calculate_portvals, port_rets
)
...
class MarketBacktest(FactorBacktest):
    def __init__(
        ...
        self.factor_rets_df = FactorBacktest(
        ...
        self.business_cycle_df = self.business_cycle_df.loc[
            rebal_dates_list[0]-DateOffset(day=1):]
        
        self.business_cycle_df.index = rebal_dates_list
        
        self.rets_with_regime = pd.concat(
            [self.month_rets_df, self.business_cycle_df], 
            axis=1)
        
        self.rets_with_regime.fillna(0, inplace=True)
        self.rets_with_regime.columns = [
            f'{self.factor}', 
            'deflation', 
            'inflation', 
            'recovery',  
        ...
        self.multi_asset_df = pd.merge(
            self.rets_with_regime, 
    ...
    def inspect_factor(self) -> dict:
        ...
        for regime in each_regime:
            metric_result[regime] = Metric(
                portfolio=(
                    1 + self.rets_with_regime[self.factor].mul(
                        self.rets_with_regime[regime], 
                        axis=0
                    )).cumprod(), 
                freq='month').numeric_metric()
    ...
    def inspect_multi_asset(self) -> dict:
        ...
        for regime in each_regime:
            ...
            for ticker in tickers:
                temp_dict[ticker] = Metric(
                    portfolio=(
                        1 + self.multi_asset_df[ticker] * self.multi_asset_df[regime]
                    ).cumprod(), 
                    freq='month').numeric_metric()
